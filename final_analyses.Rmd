...
featurePlot(x = train %>% select_if(is.numeric) %>% select(-lat,-long),
            ...
            main = list(
              "Figure 3. Scatterplot matrix: predictors vs. outcome",
              adj = 0),
            scales = list(
              x = list(cex = .1)
              )
...
Final candidate models were then compared using CV ROC distributions, and the final model was selected using the model with the lowest median CV ROC value. See Supplemental Figure S5 for information on model performance in test data.
...
Supplemental Figure S5 shows the test performance of the models using ROC curves: 
...
*3.c.i.   Test ROC value.* Upon evaluating the GLMl using our test data, the test ROC value was equal to `r roc.glm$auc[1]`.
