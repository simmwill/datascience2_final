...
Final candidate models were then compared using CV ROC distributions, and the final model was selected using the model with the lowest median CV ROC value. See Supplemental Figures S5 and S6 for information on model performance in training and test data.
...
bwplot(res,
       main = "Figure S5. Median resample metrics, training set (5-fold repeated CV)")

```

Supplemental Figure S6 shows the test performance of the models using ROC curves: 
...
title(main = "Figure S6. ROC curves for all tuned models, test data", adj = 0, line = 2.5)
